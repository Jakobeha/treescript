...
As before, there are 2 types of objects - **values** and **reducers**. A program still consists of reducers, and there's a query. For dynamic programs, the query may include or reduce to actions - e.g. to read input, the query can reduce to an expression which includes `stdin`. There are no macros or multiple phases, they're not needed. Descript no longer refactors itself directly, and if you want to write code which handles `Add`, `Subtract`, `Multiply`, and `Divide` a common way, you can do reduce these into `Arith[op: ...; left: ...; right: ...]` in the single phase.
...
- Target language code blocks are desugared. They're parsed into instances of `<target>::Node`, *using the C++ language parser library at compile-time*. This library is linked with a universal program which expects some `node.serialize...()` methods and `<target>::serializationHeader` and uses them to export the nodes into data, which the Descript compiler imports and replaces the code blocks with.
- The compiler starts with the source code to define `class Descript::Node`:
  - The compiler defines `class Descipt::Primitive` and `class Descript::Record`
  - For all record declarations, the compiler defines `class Descript::<name>(Descript::Node* <prop1>, Descript::Node* <prop2>, ...)`
  - The compiler defines an instance of `Descript::<name>` for every `<target>::Node` record.
- The compiler defines conversion functions to convert instances of `<target>::Node` to `Descript::Node`s.
- The reducers are used to create a `public virtual Descript::Node* Descript::Node::reduce()` method (in functional programming, `reduce : Descript.Node -> Maybe Descript.Node`)
  - Each reducer `N` (whose input surface is a record `<type>`) adds a new case in `<type>`'s override - `if consumeN(inp) { return produceN(inp) }` - and new methods `private bool Descript::<type>::consumeN()` and `private Descript::Node* Descript::<type>::produceN()` (in functional programming, adding a new case `reduce (<type> ...) | consumeN ... = Just (produceN ...)`). At the end of each record's override, `<type>` returns `super.reduce()`. The base class implementation, also covering primitives, returns `null` (`reduce _ = Nothing`).
  - In the record's `consumeN`, each property is checked against its corresponding input property - `return prop1.consumeNProp1() && prop2.consumeNProp2() && ...`. `public virtual Descript::Node* Descript::Node::consumeNPropM`s is defined similar to `reduce`.
...
- The main program runs as follows:
  ...
  - Parses the source code into a `<target>::Node` using the C++ parser linked *at runtime*.
  - Converts this into a `Descript::Node`.
  - Calls the node's `reduce()` method until it returns `null` (define a helper in `Node`, `reduceFull()`), which:
    - Applies the reducers from the Descript source to transform the node from the input shape into the output shape.
    - Applies target-language common reducers (can be explicitly imported by the Descript source, or implicitly defined in an external Descript source) to transform the node from the output shape into a primitive string, containing the output
    - Prints this output (specifically, to stdout)

Regular Descript values can be represented in runtime C++ via `class Descript::Node` (in functional programming, a closed variant `data Descript.Node = Primitive Descript.Primitive | Record Descript.Record`):
...
- Backend language code blocks (with splices applied) are instances of `template <typename T> class Descript::Primitive(T value) : public Descript::Node` (in functional programming, a record `data Descript.Primitive = Primitive Descript.PrimType Descript.BackendCode` or variant `data Descript.Pritive = PrimString Descript.BackendString | Prim)`
  - The primitive type is stored in `T`, e.g. `Descript::Primitive<string>` for string templates. Primitives requiring splices are reflected in the type - `Descript::Primitive<Descript::Splice<T>>`, where `template <T> struct Descript::Splice { T (*fill)(Descript::Node* inner); }`
  - Applied splices are transformed into records - `#Splice[outer: ...; inner: ...]`
- Records are instances of `class Descript::Record : public Descript::Node` (in functional programming, a compile-time open variant `open data Descript.Record abs`)
...
The C++ target language parser and `<target>::Node` are *separate* from the generated `C++` program and `Descript::Node`. The target language parser is a library linked with 2 programs, one to desugar target blocks at compile time and one to parse for generated code at runtime. The parser and `<target>::Node` can be structured however it wants, as long as the custom AST records implement `<target>::Node` and there's a global constant `<target>::serializationHeader`.
...
```descript
r'
for (\(<i=?'is_name($)') in \(<iter=?'!modifies($, x)')) {
  \(<x=?'is_name($) && $ != i')[[\(<?'$ == i')]] <- \(<expr=?'!modifies($, x)')
}
': r'
y <- as.list(x)
