...
traverseAstSelf
  :: (Monad w) => Term o -> Term i -> (i t -> w (i t)) -> o t -> w (o t)
...
traverseAstSub TValue ti f (ValuePrimitive t prim) =
  ValuePrimitive t <$> traverseAst TPrim ti f prim
traverseAstSub TValue ti f (ValueRecord t record) =
  ValueRecord t <$> traverseAst TRecord ti f record
traverseAstSub TValue ti f (ValueBind t bind) =
  ValueBind t <$> traverseAst TBind ti f bind
...
traverseAstSub TNext ti f (NextCast cast) =
  NextCast <$> traverseAst TCastRef ti f cast
...
allGroupRefReducers
  :: M.Map (Symbol ()) (GroupDef t) -> GroupRef t -> [Reducer t]
