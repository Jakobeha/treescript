...
type PFProgram = Program () () () () ()
type PFGroupDef = GroupDef () () ()
...
data GroupDef a1 a2 an
  = GroupDef
  { groupDefAnn :: an
  , groupDefValueProps :: [(a1, Bind an)]
  , groupDefGroupProps :: [(a1, Bind an)]
...
data Program a1 a2 a3 a4 an
  ...
  , programGroups :: M.Map (Symbol ()) (GroupDef a3 a4 an)
...
instance (Semigroup a1, Semigroup a2, Semigroup an) => Semigroup (Program a1 a2 a3 a4 an) where
...
-- | No path.
instance (Monoid a1, Monoid a2, Monoid an) => Monoid (Program a1 a2 a3 a4 an) where
  mempty
    = Program
    { programAnn = mempty
    , programPath = ""
...
instance (PrintableArr a1, PrintableArr a2, PrintableBid a3) => Printable (Program a1 a2 a3 a4 an) where
...
-- TODO: Print env
printGroupDef :: (PrintableBid a1) => Symbol () -> GroupDef a1 a2 an -> T.Text
printGroupDef head' (GroupDef _ vprops gprops reds _)
  ...
  where printDecl
          ...
          <> "."
        printGroupDefProp pre (bid, (Bind _ idx)) = pre <> printBid bid <> pprint idx
