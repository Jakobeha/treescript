...
convertBinaryRes :: (Monad m, MonadResult m) => Either (B.ByteString, ByteOffset, String) (B.ByteString, ByteOffset, a) -> m a
convertBinaryRes (Left (_, _, msg)) = mkFail $ desugarError_ $ "couldn't extract module: " <> T.pack msg
convertBinaryRes (Right (rem', _, prog)) = do
  unless (B.null rem') $
    tellError $ desugarError_ $ "some input left after decompiling"
  pure prog
...
decompile ser = (convertBinaryRes . decodeOrFail =<<) $ convertBinaryRes $ (`runGetOrFail` ser) $ do
  let getHeader = do
        nxt <- getWord8
        when (nxt /= 10) -- '\n' :: Word8
          getHeader
...
exportInterp outPath = liftIOAndCatch StageWriteCompiled . B.writeFile outPath . encodeInterp
