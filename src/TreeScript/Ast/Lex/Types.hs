...
instance Functor Program where
  fmap f (Program (Annd ann lexemes)) =
    Program $ Annd (f ann) (map (fmap f) lexemes)
...
instance Traversable Program where
  traverse f (Program (Annd ann lexemes)) =
    fmap Program $ Annd <$> f ann <*> traverse (traverse f) lexemes
...
instance ReducePrintable (SpliceFrag an) where
  reducePrint (SpliceFrag content isStart isEnd) =
   ...
   where
    reducePrintStart | isStart   = "'"
                     | otherwise = ""
    reducePrintEnd | isEnd     = "'"
                   | otherwise = "\\"
